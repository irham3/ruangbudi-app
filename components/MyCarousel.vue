...
const slides = ref([
  { image: '#', description: 'Dog\'s Nose' },
  { image: '#', description: 'Lawnmower' },
  { image: '#', description: 'Globe' },
  { image: '#', description: 'Optical Illusion' },
  { image: '#', description: 'Lips' },
])
const caption = ref('')
  ...
  <section class="container mx-auto">
    <div class="bg-cover bg-center h-auto w-full">
      <div v-for="(slide, index) in slides" :key="index" class="mySlides">
        <div :class="`image${index + 1} w-full object-cover`" />
      </div>

      <!-- buttons -->
      <a class="absolute left-0 inset-y-0 flex items-center -mt-32 px-4 text-white hover:text-gray-800 cursor-pointer text-3xl font-extrabold" @click="plusSlides(-1)">❮</a>
      <a class="absolute right-0 inset-y-0 flex items-center -mt-32 px-4 text-white hover:text-gray-800 cursor-pointer text-3xl font-extrabold" @click="plusSlides(1)">❯</a>
      ...
      <div class="flex">
        <div v-for="(slide, index) in slides" :key="index">
          <img :class="`image${index + 1} description h-24 opacity-50 hover:opacity-100 cursor-pointer`" :src="slide.image" :alt="slide.description" @click="currentSlide(index + 1)">
        </div>
      </div>
    </div>
  </section>
